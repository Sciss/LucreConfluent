...
//      val group: TxnCacheGroup[ Long, Path[ X ]]
      ...
      protected def addDirty( hash: Pth )( implicit access: C ) : Unit
      protected def addAllDirty( hashes: Traversable[ Pth ])( implicit access: C ) : Unit

      def put( key: Pth, value: V )( implicit access: C ) {
         ref.transform( map => {
//            if( map.isEmpty ) rec.addDirty( this )
            val hash = key.sum
            addDirty( key ) // group.addDirty( this, hash )
            map + (hash -> (key, value))
         })( access.txn )
      }
...
//            val hashed = keys.map( _.sum ) // elems.view.map( tup => (tup._1.sum, tup._2) )
            addAllDirty( elems.map( _._1 )) // group.addAllDirty( this, hashed )
...
//      }
   ...
   private class ValCache[ C <: Ct[ C ], X, V ]( protected val store: TxnStore[ C, PathLike[ X ], V ],
                                                 group: TxnCacheGroup[ C, PathLike[ X ], PathLike[ X ]])
   extends CacheLike[ C, X, V ] with TxnCacheLike[ C, PathLike[ X ]] {
      ...
      protected def addAllDirty( hashes: Traversable[ Pth ])( implicit access: C ) {
...
//   }
   ...
   def valFactory[ C <: Ct[ C ], X ]( group: TxnCacheGroup[ C, PathLike[ X ], PathLike[ X ]]) : TxnDelegateValStoreFactory[ C, PathLike[ X ], Any ] =
