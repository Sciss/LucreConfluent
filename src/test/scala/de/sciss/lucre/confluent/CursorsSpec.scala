...
//   confluent.showLog = true
   ...
   object Entity {
      implicit object CursorSer extends stm.Serializer[ D#Tx, D#Acc, Cursor[ S ]] {
         def write( c: Cursor[ CursorsSpec.this.type#S ], out: DataOutput ) { c.write( out )}
         def read( in: DataInput, access: D#Acc)( implicit tx: D#Tx ) : Cursor[ S ] = {
   ...
   class Entity( val id: S#ID, val field: S#Var[ Int ], cursorsVar: D#Var[ IIdxSeq[ Cursor[ S ]]])
