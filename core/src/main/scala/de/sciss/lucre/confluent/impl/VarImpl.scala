...
private[impl] sealed trait BasicVar[S <: Sys[S], A] extends Var[S, A] {
  ...
  final def write(out: DataOutput): Unit = out.writePackedInt(id.base)  // writeInt
...
private[impl] final class IntVar[S <: Sys[S]](protected val id: S#ID)
  ...
  // ---- Serializer ----
  def write(v: Int, out: DataOutput): Unit = out.writePackedInt(v) // out.writeInt(v)

  def read(in: DataInput): Int = in.readPackedInt() // in.readInt()
}
