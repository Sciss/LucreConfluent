...
object Model {
   ...
   def onCommit[ C <: Ct[ C ], T ]( committed: Traversable[ T ] => Unit ) : Listener[ C, T ] =
      filterOnCommit( (_: T, _: C) => true )( committed )

   def filterOnCommit[ C <: Ct[ C ], T ]( filter: Function2[ T, C, Boolean ])( committed: Traversable[ T ] => Unit ) =
      new Listener[ C, T ] {
   ...
   def reduceOnCommit[ C <: Ct[ C ], T, U ]( pf: PartialFunction[ (T, C), U ])( committed: U => Unit ) =
      new Listener[ C, T ] {
         val lifted = pf.lift
...
trait Model[ C, +T ] {
   ...
   def addListener( l: Listener[ C, T ])( implicit c: Ct[ _ ]) : Unit
   def removeListener( l: Listener[ C, T ])( implicit c: Ct[ _ ]) : Unit
}
