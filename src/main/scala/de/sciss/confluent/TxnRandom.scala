...
import de.sciss.lucre.stm.{Sys, Var}
...
object TxnRandom {
   ...
   def apply[ S <: Sys[ S ]]( id: S#ID )( implicit tx: S#Tx ) : TxnRandom[ S#Tx ] =
...
//         haveNextNextGaussian = false
   ...
   private final class PlainImpl( seedRef: Ref[ Long ]) extends Impl[ InTxn ] {
      ...
      protected def refGet( implicit tx: InTxn ) : Long = seedRef.get
   }

   private final class SysImpl[ Txn ]( seedRef: Var[ Txn, Long ]) extends Impl[ Txn ] {
      protected def refSet( value: Long )( implicit tx: Txn ) {
         seedRef.set( value )
      }

      protected def refGet( implicit tx: Txn ) : Long = seedRef.get
   }
